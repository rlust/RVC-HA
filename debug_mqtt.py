...
# Create log file
with open('debug_mqtt.log', 'w') as f:
    f.write(f"Debug log created at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
...
def log_message(message):
    ...
    print(log_entry)
    with open('debug_mqtt.log', 'a') as f:
        f.write(log_entry + "\n")
...
try:
    # Prepare the sink light command
    instance_id = 46
